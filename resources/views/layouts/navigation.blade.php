    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @auth
                    <!-- Cart Link -->
                    <a href="{{ route('cart.index') }}" class="mr-4 text-gray-500 hover:text-amber-600 relative">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                        </svg>
                            @php $cartCount = array_sum(session('cart', [])); @endphp
                                <span class="absolute -top-2 -right-2 bg-red-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center {{ $cartCount ? '' : 'hidden' }}" data-cart-count>
                                    {{ $cartCount }}
                                </span>
                    </a>
                ...
                @else
                    <div class="space-x-4">
                        <a href="{{ route('login') }}" class="text-sm font-semibold text-gray-600 hover:text-amber-600">Log in</a>
            ...
            <div class="-me-2 flex items-center sm:hidden">
                @auth
                <a href="{{ route('cart.index') }}" class="mr-2 text-gray-500 hover:text-amber-600 relative">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"></path>
                    </svg>
                    @php $cartCount = array_sum(session('cart', [])); @endphp
